...
void setup() {
  ...
  pinMode(botao4, INPUT_PULLUP); // Botão 4 como entrada com pull-up
...
void loop() {
  ...
  int estadoBotao4 = digitalRead(botao4); // Lê botão 4
  ...
  if (estadoBotao4 == LOW) pressionados++;

  // Acende LED se exatamente dois botões forem pressionados
  if (pressionados == 2) {
